...
DWORD WINAPI ClientThread(PVOID pvParam) {
   ...
   for (int nRequestNum = 1; !g_fShutdown; nRequestNum++) {
      ...
      TCHAR sz[1024];
      DWORD timePlaced = GetTickCount();
      CQueue::ELEMENT e = { nThreadNum, nRequestNum, timePlaced };
...
DWORD WINAPI ServerThread(PVOID pvParam) {
   ...
   while (!g_fShutdown) {
      ...
      if (g_q.Remove(&e, 5000)) {
         ...
         DWORD delta = (currentTime - e.timePlacedInQ) / 1000;
         wsprintf(sz, TEXT("%d: Processing %d:%d took %d seconds"), 
            nThreadNum, e.m_nThreadNum, e.m_nRequestNum, delta);
